    ...
    for group_id in config.GROUP_ID:
        try:
            if datetime.now().hour == 8 and datetime.now().minute == 0:
                await bot.send_group_msg(group_id=group_id, message="指挥官休息的还好吗，拉菲现在很有精神，大概")
            elif datetime.now().hour == 23 and datetime.now().minute == 30:
                await bot.send_group_msg(group_id=group_id, message="呼啊...指挥官，要一起睡一会儿吗？")
            else:
                await bot.send_group_msg(group_id=group_id, message=time_msg + msg)
        except e:
            print(e)
            await bot.send_group_msg(group_id=group_id, message="唔……嗯……糟了，站着睡着了")
...
async def hint_message():
    ...
    try:
        await bot.send_group_msg(group_id=config.GROUP_ID[0], message=msg)
    except e:
        await bot.send_group_msg(group_id=config.GROUP_ID[0], message=e)
