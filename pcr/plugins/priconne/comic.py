...
async def comic(bot, ev: CQEvent):
    ...
    if not episode:
        await bot.send(ev, '请输入漫画集数 如：漫画132', at_sender=True)
        return
    index = load_index()
    if episode not in index:
        await bot.send(ev, f'未查找到第{episode}话喵...', at_sender=True)
        return
...
async def update_seeker():
    ...
    await sv.broadcast(msg, 'PCR四格漫画~', 0.5)
